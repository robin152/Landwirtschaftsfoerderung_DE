...
function ProblemCard({ problem, index }: { problem: (typeof problems)[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`relative bg-white rounded-2xl border border-slate-200 border-l-4 ${problem.accent} shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden`}
    >
      <div className="p-6 sm:p-8">
        ...
        {/* Title */}
        <h3 className="text-base sm:text-lg font-bold text-slate-900 leading-snug mb-3 text-balance">
          {problem.title}
        </h3>

        {/* Body */}
        <p className="text-sm sm:text-base text-slate-500 leading-relaxed">{problem.body}</p>
      </div>
    </motion.div>
  )
}
    ...
    <>
      ...
      <section className="relative bg-slate-50 py-20 sm:py-28 overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          ...
          >
            <span className="inline-block text-xs font-bold uppercase tracking-[0.2em] text-red-500 mb-5">
              Die Realität auf deutschen Höfen
            </span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 leading-[1.1] text-balance mb-5">
          ...
          >
            <div className="w-8 h-px bg-slate-300 mx-auto mb-6" />
            <blockquote className="text-xl sm:text-2xl font-semibold text-slate-700 italic leading-relaxed text-balance">
              "Ich wusste nicht mal, dass ich Anspruch hatte. Vier Jahre lang hab ich das Geld liegen lassen."
            </blockquote>
            <p className="mt-4 text-sm text-slate-400 font-medium">— Milchviehhalter, Bayern, 2024</p>
          </motion.div>
        </div>
      </section>

      {/* Bridge to solution — light */}
      <div ref={bridgeRef} className="relative bg-white border-t border-slate-100 py-16 sm:py-20 overflow-hidden">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl text-center">
          <motion.div
            initial={{ scaleY: 0, opacity: 0 }}
            animate={bridgeInView ? { scaleY: 1, opacity: 1 } : {}}
            transition={{ duration: 0.45, delay: 0.1 }}
            className="w-px h-12 bg-gradient-to-b from-slate-200 to-emerald-500 mx-auto mb-8 origin-top"
          />
          <motion.p
            initial={{ opacity: 0, y: 10 }}
            animate={bridgeInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.45, delay: 0.3 }}
            className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-600 mb-5"
          >
            Die gute Nachricht
          </motion.p>
          <motion.h2
            ...
            className="text-2xl sm:text-3xl lg:text-4xl font-black text-slate-900 leading-tight text-balance"
          >
            Für all das gibt es staatliche Förderung —
            <br />
            <span className="text-emerald-600">und wir holen sie für dich.</span>
          </motion.h2>
          <motion.div
            initial={{ scaleY: 0, opacity: 0 }}
            animate={bridgeInView ? { scaleY: 1, opacity: 1 } : {}}
            transition={{ duration: 0.45, delay: 0.65 }}
            className="w-px h-10 bg-gradient-to-b from-emerald-500 to-slate-200 mx-auto mt-8 origin-top"
          />
        </div>
      </div>
    </>
